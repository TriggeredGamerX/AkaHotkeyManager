package akatriggered.optimizer.command;
...
public class EnableOptimizerCommand {
    ...
    public void initializeToggleCommands() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) ->
                dispatcher.register(ClientCommandManager.literal("akafastcrystal").executes(context -> {
                    if (fastCrystal) {
                        fastCrystal = false;
                        displayMessage("Aka's Fast crystals disabled!");
                    } else if (!fastCrystal) {
                        fastCrystal = true;
                        displayMessage("Aka's Fast crystals enabled");
                    }
                    return 1;
                }))
        );
